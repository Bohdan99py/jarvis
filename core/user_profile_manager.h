...
// current focus (developer, creative, admin, casual) and
// injects an adaptive system instruction into SessionMemory.
// -------------------------------------------------------
...
enum FocusState { Developer, Creative, Admin, Casual };
...
inline QString focusInstruction(FocusState f)
{
    ...
    case Creative:
        return QStringLiteral(
            "User is in a creative/design workflow. "
...
inline FocusState detectFocus(qint64 userId)
{
    ...
    int devScore = 0, creativeScore = 0, adminScore = 0;
    ...
    if (devScore >= creativeScore && devScore >= adminScore && devScore >= 4)
        return Developer;
    if (creativeScore >= devScore && creativeScore >= adminScore && creativeScore >= 4)
        return Creative;
    if (adminScore >= devScore && adminScore >= creativeScore && adminScore >= 4)
        return Admin;
    return Casual;
}
