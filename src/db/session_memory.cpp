...
QString SessionMemory::buildSystemPrompt() const
{
    ...
    // --- Активный режим работы (ModeManager) ---
    // Стоит ДО скилл-блоков намеренно: контекст «что сейчас происходит»
    // задаёт рамку, в которой скилл-знания интерпретируются. Например,
    // «режим Беседа» + активный скилл-кодер = разговор с человеком,
    // умеющим программировать, но не рабочий IDE-сеанс.
    if (!m_modeContext.isEmpty()) {
        prompt += m_modeContext;
    }
